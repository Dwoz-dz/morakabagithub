-- Phase 1 (Critical Fixes) - Full standalone migration
-- Covers:
-- 1) Employee profile sync and self-heal RPCs
-- 2) Profile avatar storage flow
-- 3) Weapon signature path persistence
-- 4) Vehicle image support for fuel selector UI
--
-- Safe to run in Supabase SQL Editor (idempotent as much as possible).
...
-- ------------------------------------------------------------
-- B) Profile sync functions (auth.uid() -> employees.auth_user_id)
-- ------------------------------------------------------------
...
-- Backfill existing registration requests -> employees
do $$
declare
  r record;
begin
...
-- Keep explicit select policy for profile loading
...
-- ------------------------------------------------------------
-- D) Storage: vehicle-images (Fuel selector circles)
-- ------------------------------------------------------------
